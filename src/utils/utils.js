...
$.fn.isAlmostScrolledInContainerBottomToTop = function(container, offset) {
    if (!this.length || !container.length)
        return;
    offset = offset || 0;
    container = container[0];
...
var utils = {
    ...
    generateDeviceName: function () {
        ...
        let random_name = `${adjectives[Math.floor(Math.random()*adjectives.length)]} ${animals[Math.floor(Math.random()*animals.length)]}`
        return random_name;
    },
