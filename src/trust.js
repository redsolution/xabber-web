...
xabber.Trust = Backbone.ModelWithStorage.extend({
    ...
    publishOwnTrustedDevices: function (callback) {
        ...
        let iq = $iq({type: 'set'})
            ...
            .c('items', {timestamp: current_timestamp});
        ...
        let $trusted_items = $(iq.tree()).find('items');


        $trusted_items.children().sort(function(a, b) {
        ...
        $trusted_items.children().each((idx, item) => {
            let $item = $(item);
    ...
    publishContactsTrustedDevices: function (difference) {
        let trusted_devices = difference ? difference :this.get('trusted_devices');
        ...
        stanza.c(`${share_type}`, {xmlns: Strophe.NS.PUBSUB_TRUST_SHARING, usage: Strophe.NS.OMEMO});
        ...
        Object.keys(trusted_devices).forEach((item) => {
            ...
            trusted_devices[item].forEach((device_item) => {
                if (device_item.is_revoked){
                    ...
                    stanza.c('distrust', {timestamp: device_item.timestamp}).t(device_item.trusted_key).up();
                } else {
                    stanza.c('trust', {timestamp: device_item.timestamp}).t(device_item.trusted_key).up();
                }
                counter++;
            });
            stanza.up();
        ...
        let $share = $(stanza.tree()).find(`${share_type}`),
            trusted_string = '';
        ...
        $share.children('items').each((idx, trusted_items) => {
            ...
            $item.children().each((idx, trust_item) => {
    ...
    iterateAndChangeTrustedDevices: function (jid, func) {
        ...
        jid_trusted_devices && jid_trusted_devices.length && jid_trusted_devices.forEach((trusted_device, index) => {
            ...
            if (Boolean(func_result.func_changed)){
                ...
                jid_trusted_devices[index] = trusted_device;
            }
        });

        if (changed){
            let whole_trusted_devices = this.get('trusted_devices');

            let deviceMap = new Map();
            whole_trusted_devices[jid].forEach(item => {
                deviceMap.set(item.trusted_key, { ...item });
            });
            jid_trusted_devices.forEach(item => {
                deviceMap.set(item.trusted_key, { ...item });
            });
            whole_trusted_devices[jid] = Array.from(deviceMap.values());

            this.save('trusted_devices', whole_trusted_devices);
            this.trigger('trust_updated');
        }
        return changed;
    ...
    findAndMarkRemovedTrustedDevices: function (devices_ids, peer_jid, callback, revocation_timestamp) {
    ...
    fixMyTrustedDeviceAndPublish: function (callback) {
        let trusted_devices = this.get('trusted_devices'),
    ...
    getVerificationStateLabel: function (session) {
        let state,
            step = session.verification_step;
        if (step === '1a' && session.active_verification_device && session.active_verification_device.device_id){
            state = xabber.getString("verification_session_state__device_verification");
        } else if (step === '1a'){
            state = xabber.getString("verification_session_state__device_verification");
        } else if (step === '1b'){
            state = xabber.getString("verification_session_state__device_verification");
    ...
    getVerificationStateLabelTrue: function (session) {
        let state,
            step = session.verification_step;
        if (step === '1a' && session.active_verification_device && session.active_verification_device.device_id){
    ...
    findDifferenceInTrustedDevices: function (trustedDevices, changedTrustedDevices) {
        let difference = {};

        for (let jid in changedTrustedDevices) {
            if (trustedDevices[jid]) {
                let originalList = trustedDevices[jid],
                    changedList = changedTrustedDevices[jid];

                let diffs = [];

                for (let changedDevice of changedList) {
                    let originalDevice = originalList.find(device => device.device_id === changedDevice.device_id);

                    if (!originalDevice) {
                        // Device is new
                        diffs.push(changedDevice);
                    } else {
                        let diff = { ...changedDevice },
                            isDifferent = false;

                        for (let key in changedDevice) {
                            if (changedDevice[key] !== originalDevice[key]) {
                                isDifferent = true;
                                break;
                            }
                        }
                ...
                if (diffs.length > 0) {
                    difference[jid] = diffs;
                }
            } else {
                // All devices are new
                difference[jid] = changedTrustedDevices[jid];
            }
        }
    ...
    clearData: function (sid, no_answer) {
        let active_sessions = this.get('active_trust_sessions'),
            session;

        session = active_sessions[sid];
        if (session && session.incoming_request_data && !no_answer){
            let message_options = session.incoming_request_data.message_options;
            message_options && message_options.archive_id && this.account.answerPriorityMessage(message_options.archive_id);
        }
    ...
    getNewTrustedDevices: function (trusted_devices, $message, final_trusted_devices, is_first, peer, has_changes, initial_trusted_devices) {
        has_changes = has_changes || false;
        initial_trusted_devices = initial_trusted_devices || JSON.parse(JSON.stringify(this.get('trusted_devices')));
        ...
        if (!trusted_devices.length && !is_first && final_trusted_devices.length && was_removed){
            ...
            if (peer){
                ...
                this.publishContactsTrustedDevices(difference);
            } else {
                this.fixMyTrustedDeviceAndPublish();
            }
        }
        ...
        trusted_devices.forEach((item) => {
            ...
            dfd.done(() => {
                ...
                if (counter === trusted_devices.length) {
                    ...
                    if (new_trusted_devices.length){
                        this.getNewTrustedDevices(new_trusted_devices, $message, final_trusted_devices, null, peer, has_changes, initial_trusted_devices)
                    } else {
            ...
            $trusted_items.children().each((idx, trust_item) => {
                let $item = $(trust_item);
                trusted_string = trusted_string + `<${$item.attr('timestamp')}/${$item.text()}`;
            });
    ...
    isDeviceTrusted: function (jid, device_id) {
        ...
        if (!trusted_devices[jid])
            return false;
        return trusted_devices[jid].some(e => e.device_id === device_id);


    },

    receiveTrustVerificationHeadline: function (message, options) {
        let $message = $(message),
        ...
        if (this.active_sessions_data[sid]){
            if (!this.active_sessions_data[sid].verification_step){
                ...
                this.clearData(sid, true);
            } else {
                options.archive_id && this.account.answerPriorityMessage(options.archive_id);
            }
        } else {
            if ($message.find(`verification-accepted`).length) {
                options.archive_id && this.account.answerPriorityMessage(options.archive_id);
            }
        }
    },
    ...
    receiveTrustVerificationMessage: function (message, options) {
        ...
        if (type === 'headline')
            return this.receiveTrustVerificationHeadline(message, options);
        ...
        if (contact){
            if ($message.find('verification-start').length && $message.find('verification-start').attr('device-id') && this.omemo.get('device_id') && options.automated){
                ...
                let msg_timestamp = $message.find('authenticated-key-exchange').attr('timestamp'),
                    ttl;
                if (msg_timestamp) {
                    if ($message.find('verification-start').attr('ttl')){
            ...
            this.generateVerificationArrayBuffer(devices_IK.device_pubkey, devices_IK.own_privkey, code).then((response) => {
                ...
                this.account.sendFast(stanza, () => {
                    if (contact){
                        ...
                        let new_msg_id = uuid();
                        $stanza.attr('id', new_msg_id);
                        $stanza.attr('type', 'headline');
                        $stanza.children('body').remove();
                        $stanza.find('verification-accepted').attr('device-id', this.account.omemo.get('device_id'));
                        $stanza.find('salt').remove();
                        stanza = stanza.tree().cloneNode(true);
                        this.account.sendFast(stanza, () => {
                        });
                    }
    ...
    sendTrustedDevices: function (contact, device, sid) {
        this.getTrustedKey(device).then((trustedKeyBuffer) => {
                ...
                if (contact && trusted_devices[to].some(e => e.trusted_key === trustedKeyBase64 && e.untrusted)) {
                    changed = this.iterateAndChangeTrustedDevices(to, (trusted_device) => {
                        ...
                        if (trusted_device.device_id === device.get('id') && trusted_device.untrusted && !trusted_device.is_revoked) {
                            ...
                            func_changed = true;
                            changed_trusted_devices[to] = [trusted_device];
                        }
                        return {
                    ...
                    changed_trusted_devices[to] = [item];
            ...
            if (contact){
                ...
                this.publishContactsTrustedDevices(changed_trusted_devices); // передать списком устройств
            } else {
                this.fixMyTrustedDeviceAndPublish(() => {
