...
let env = xabber.env,
    ...
    opusRecorder = env.opusRecorder,
    opusRecorderEncoderPath = env.opusRecorderEncoderPath,
  ...
  xabber.Messages = Backbone.Collection.extend({
    ...
    createInvitationFromStanza: function ($message, options) {
        ...
        let $invite_item = $message.find('invite'),
            ...
            inviter_jid = Strophe.getBareJidFromJid($message.attr('from')),
    ...
    createFromStanza: function ($message, options, account) {
        ...
        if (options.high_priority){
            let $keyExchange = $message.children(`authenticated-key-exchange[xmlns="${Strophe.NS.XABBER_TRUST}"]`);
        ...
        let references;
        if (options.notification_msg && $notification_msg){
            references = $notification_msg.children(`reference[xmlns="${Strophe.NS.REFERENCE}"]`);
        } else {
            references = $message.children(`reference[xmlns="${Strophe.NS.REFERENCE}"]`).length ?
        ...
        if ((options.replaced || options.encrypted && options.is_unread_archived) && message) {
            message.set(attrs);
            if (options.update_replaced){
                options.update_replaced();
                options.update_replaced = null;
            }
            return;
        }
    ...
    getFileType: function(full_type) {
        ...
        return full_type.slice(0, end_idx);
    },
    ...
    onChangedLastReadMsg: function (options) {
        if (this.get('prev_last_read_msg') && this.get('last_read_msg') && this.get('prev_last_read_msg') !== this.get('last_read_msg')){
            ...
            let query = {
                ...
                is_first: true,
                sync_update: !!(options && options.sync_update),
                var: [
      ...
      receiveMessage: function (msg_object) {
          ...
          if ($message.find('invite').length) {
              ...
              if (this.account.connection && this.account.connection.do_synchronization) {
                  if (msg_object.synced_msg || !msg_object.synced_msg && !msg_object.is_archived)
                      return this.messages.createInvitationFromStanza($message, msg_object);
              } else {
                  let iq = $iq({type: 'get'}).c('blocklist', {xmlns: Strophe.NS.BLOCKING});
                  this.account.sendIQFast(iq, (iq) => {
                      let items = $(iq).find('item'),
                          current_timestamp = Number(moment($message.find('delay').attr('stamp') || $message.find('time').attr('stamp') || (msg_object.delay) && Number(moment(msg_object.delay.attr('stamp'))) || moment.now())),
              ...
              return this.messages.createFromStanza($message, msg_object);
          }
      },
            ...
            if (stanza_id || contact_stanza_id) {
                let iq_retraction = $iq({type: 'set', to: group_chat ? (this.contact.get('full_jid') || this.contact.get('jid')) : this.account.get('jid')})
                    .c('retract-message', {
                        ...
                        conversation: this.account.server_features.get(Strophe.NS.XABBER_FAVORITES) && this.get('saved') ? this.account.server_features.get(Strophe.NS.XABBER_FAVORITES).get('from') : this.contact.get('jid'),
                        symmetric: symmetric,
...
xabber.ChatItemView = xabber.BasicView.extend({
    ...
    updateIcon: function () {
        ...
        if (this.contact.get('private_chat') && typeof(this.contact.get('private_chat')) === 'string'){
            ...
            let contact = this.account.contacts.get(this.contact.get('private_chat'));
            if (contact){
                this.$('.chat-icon').html('');
    ...
    updateNotificationsState: function () {
        let is_notifications = this.model.get('notifications');
        if (is_notifications || (!is_notifications && xabber.toolbar_view.$('.active').hasClass('mentions')))
            this.$el.detach();
        if (is_notifications && xabber.toolbar_view.$('.active').hasClass('mentions') || !is_notifications && !xabber.toolbar_view.$('.active').hasClass('mentions'))
            xabber.chats_view.updateChatPosition(this.model);
    },
    ...
    onItemContextMenu: function (ev) {
        ...
        $modal.find('.btn-read').one(`click.${unique_modal_id}`, () => {
            if (!this.model.get('saved')) {
        ...
        $modal.find('.btn-delete-chat').one(`click.${unique_modal_id}`, () => {
            if (!this.model.get('saved'))
  ...
  xabber.MessageContextView = xabber.MessagesView.extend({
      ...
      scrollToUnreadWithButton: function () {
          this.openChat();
          this.chat_content.scrollToUnread();
          this.chat_content.$('.back-to-unread').addClass('back-to-bottom');
      },
  ...
  xabber.SearchedMessagesView = xabber.MessagesView.extend({
      ...
      messagesRequest: function (query, callback) {
          ...
          this.chat_content.MAMRequest(options, (success, messages, rsm) => {
                  ...
                  $(messages).each((idx, message) => {
                      let $message = $(message);
                      this.account.chats.makeMessageObject($message, {searched_message: true}).then(() => {
                          count++;
                          if (count === messages.length) {
                              callback && callback();
                          }
  ...
  xabber.ContactSearchedMessagesView = xabber.MessagesView.extend({
      ...
      messagesRequest: function (query, timestamp, rsm, loaded_messages, callback) {
          ...
          this.chat_content.MAMRequest(options, (success, messages, rsm) => {
                  ...
                  if (messages.length === options.max){
                      if (this.parent &&  this.parent.data && !this.parent.data.get('visible') || (this.parent.model && this.parent.model.get('search_hidden')))
                          return;
                      else
      ...
      addMessageHTML: function ($message, length, index) {
          if (index === 0)
              $message.appendTo(this.$('.chat-content'));
          else {
              $message.insertBefore(this.$('.chat-message').eq(length - index - 1));
          }
          return $message;
      },
      ...
      addMessage: function (message) {
          if (!this.parent.model || !this.parent.model.get('search_hidden')) {
              ...
              this.addMessageHTML($message, this.account.searched_messages.length, index);
          }
      },
...
xabber.ChatContentView = xabber.BasicView.extend({
    ...
    updateActiveSessionHeight: function () {
        ...
        let height = this.$('.notification-trust-session').height() + 64;
        if (!this.$('.notification-trust-session').height()){
            height = 0;
        }
    ...
    readVisibleMessages: function (is_context) {
        ...
        if (self.$('.chat-message.unread-message').length && xabber.get('focused') && !xabber.get('idle')){
            ...
            if (mention_ids && mention_ids.length){
                let mention_id_item = mention_ids.find(item => item.msg_id == $(last_visible_unread_msg).data('uniqueid'))
                if (mention_id_item)
                    is_unread_mention = true;
            }
            if (is_unread_mention && !$(last_visible_unread_msg).isFullyVisibleInContainer(self.$('.chat-content'))){
                last_visible_unread_msg = $(last_visible_unread_msg).prev('.chat-message.unread-message').length && $(last_visible_unread_msg).prev('.chat-message.unread-message')[0];
                if (!(last_visible_unread_msg && $(last_visible_unread_msg).isVisibleInContainer(self.$('.chat-content')))) {
                    return;
                }
            }
            if (last_visible_unread_msg && this.model.messages.get($(last_visible_unread_msg).data('uniqueid'))){
                ...
                this.readMessage(this.account.context_messages.get($(last_visible_unread_msg).data('uniqueid')), $(last_visible_unread_msg), is_context);
            } else {
    ...
    readVisibleMentions: function (is_context) {
        ...
        if (self.$('.chat-message').length && xabber.get('focused') && !xabber.get('idle')){
            ...
            _.each(mention_ids, (item) => {
                ...
                if ($msg.length && $msg.isFullyVisibleInContainer(self.$('.chat-content'))){
                    let chat_message = this.model.messages.get(item.msg_id);
                    if (chat_message)
                        chat_message.set('is_mention_msg', true);
    ...
    MAMRequest: function (options, callback, errback) {
        let account = this.account,
            ...
            is_groupchat = contact && contact.get('group_chat'), success = true, iq, handler;
        delete options.fast;
        if (is_groupchat)
                    ...
                    if (loaded_message) {
                        ...
                        if (this.model.get('notifications') && this.model.get('last_read_msg')){
                            ...
                            if (last_read_msg_item) {
                                let unread_msgs = this.model.messages.filter(m => m.get('timestamp') > last_read_msg_item.get('timestamp') && !m.get('is_unread'));
                                unread_msgs.forEach(message => {
                                    message.set('is_unread_archived', true);
    ...
    OGPLinkOnload: function ($message) {
        let $copy_link_icon = $message.find('.mdi-link-variant');
        $copy_link_icon.attr({
            'data-image': 'true'
        });
    ...
    receiveNoTextMessage: function ($message) {
    ...
    updateScrollAfterMessage: function (is_scrolled_to_bottom, scrolled_from_bottom, message) {
        if (this.model.get('notifications')){
            ...
            if ((is_scrolled_to_bottom && is_scrolling_needed) || message && message.get('submitted_here')) {
                this.scrollToBottom();
    ...
    onMessage: function (message) {
        ...
        if (this.isVisible() && (!message.get('is_unread') || is_scrolled_to_bottom) && !message.get('is_between_anchors')) {
            this.updateScrollAfterMessage(is_scrolled_to_bottom, scrolled_from_bottom, message);
        }
        ...
        if (message.get('high_priority') && !message.get('synced_from_server')){
            ...
            if (message.get('notification_msg')){
                if (message.get('notification_trust_msg')) {
                    if ($original_stanza.find('verification-successful').length){
                        message.set('message', xabber.getString("notifications_successful_verification_msg"));
                    }
                }
            }
            if (message.get('notification_trust_msg') || $original_stanza.children(`authenticated-key-exchange[xmlns="${Strophe.NS.XABBER_TRUST}"]`).length) {
                if (this.account.omemo && this.account.omemo.xabber_trust){
                    if (message.get('device_id')){
                        this.account.omemo.xabber_trust.addToSequentialProcessingList($original_stanza[0], {
                            automated: true,
                    ...
                    this.account.answerPriorityMessage(message.get('archive_id') || message.get('stanza_id'));
                }
                message.set('is_unread', false);
    ...
    removeAllMessagesExceptLast: function () {
        ...
        this.model.messages.forEach((message, idx) => {
            ...
            $message && $message.prev('.chat-day-indicator').remove();
    ...
    renderVoiceMessage: function (element, file_url, chat, file, file_key) {
        ...
        if (file){
            peaks = file.peaks;
            duration = utils.pretty_duration(file.duration);
        }
        ...
        aud.on('ready', () => {
            !duration && (duration = utils.pretty_duration(Math.round(aud.getDuration())));
        ...
        try{
            if (this.account.server_features.get('proxy-viewer') && this.account.get('proxy_viewer_url') && this.account.get('proxy_viewer_token') && file_url.includes && !file_url.includes('blob')){
                this.account.getProxyUrl(file_url, (response) => {
                    ...
                    if (file_key){
                        this.model.messages.decryptFile(url, file_key).then((result) => {
                            if (result === null)// smth about how file not decrypted
                                return;
                            if (peaks){
                                let x = aud.load(result, peaks.split(' '));
                            } else {
                                aud.load(result);
                            }
                        });
                        ...
                        if (peaks){
                            let x = aud.load(url, peaks.split(' '));
                        } else {
                            aud.load(url);
                        }
                    }
    ...
    replaceVideoThubmnailsToProxy: function (original_files, callback) {
        let files = JSON.parse(JSON.stringify(original_files)),
            dfd = new $.Deferred();
        dfd.done(() => {
            callback && callback(files)
        });
        let counter = 0;
        let onFileLoop = (file, idx) => {
            let no_touch;
            _.each(file.sources, (source) => {
            ...
            if (no_touch){
                counter++;
                if (counter === original_files.length)
                    dfd.resolve();
                return;
            }

            file.key = original_files[idx].key;
            file.proxy_video = true;

            if (file.thumbnail){
                this.account.getProxyUrl(file.thumbnail, (response) => {
                    if (!response || !response.url) {
                        counter++;
                        if (counter === original_files.length)
                            dfd.resolve();
        ...
        _.each(files, onFileLoop);
    },

    replaceSourcesToPlaceholder: function (original_files) {
        if (!original_files || !original_files.length)
        ...
        if (this.account.server_features.get('proxy-viewer') && this.account.get('proxy_viewer_url') && this.account.get('proxy_viewer_token')){
            files = JSON.parse(JSON.stringify(original_files));
            _.each(files, (file, idx) => {
                if (!file.original_sources){
                    let proxy_sources_list = [],
                        no_touch;
                    _.each(file.sources, (source) => {
                        if (source.includes('blob')) {
                            no_touch = true
                        ...
                        proxy_sources_list.push(`${this.account.get('proxy_viewer_url')}proxy/geturl/?url=${source}`)
                    });
                    if (no_touch)
                        return;
                    file.sources = [xabber.cache.placeholder_loading];
                    file.upload_id = idx;
                    file.key = original_files[idx].key;
                    file.proxy_sources_list = proxy_sources_list
                }
            });
        }
        return files;
    ...
    loadAndReplaceImagesToProxy: function (files, $message, is_encrypted, fwd_images) {
        ...
        let onFileLoop = (file, idx, list) => {
            ...
            if (list.length === 1){
                $message.find(`.uploaded-img`).css({'object-fit': 'contain'});
            } else {
                $message.find(`div[data-upload-file-id="${file.upload_id}"] img`).css({'object-fit': 'contain'});
            }
            this.account.getProxyUrl(file.proxy_sources_list[0], (response) => {
                if (!response || !response.url) {
                    return;
                }
                let url = response.url;
                if (file.key){
                    this.model.messages.decryptFile(url, file.key).then((result) => {
                        if (result === null)// smth about how file not decrypted
                            return;

                        if (list.length === 1){
                            $message.find(`.uploaded-img`).attr('data-mfp-src', result);
                            $message.find(`.uploaded-img`).attr('src', result);
                            $message.find(`.uploaded-img`)[0] && ($message.find(`.uploaded-img`)[0].onload = () => {
                                $message.find(`.uploaded-img`).css({'object-fit': ''});
                            });
                        } else {
                            $message.find(`div[data-upload-file-id="${file.upload_id}"] img`).attr('data-mfp-src', result);
                            $message.find(`div[data-upload-file-id="${file.upload_id}"] img`).attr('src', result);
                            $message.find(`div[data-upload-file-id="${file.upload_id}"] img`)[0] && ($message.find(`div[data-upload-file-id="${file.upload_id}"] img`)[0].onload = () => {
                                $message.find(`div[data-upload-file-id="${file.upload_id}`).css({'object-fit': ''})
                            });
                        }
                    });
                } else {
                    if (list.length === 1){
                        $message.find(`.uploaded-img`).attr('data-mfp-src', url);
                        $message.find(`.uploaded-img`).attr('src', url);
                        $message.find(`.uploaded-img`)[0] && ($message.find(`.uploaded-img`)[0].onload = () => {
                            $message.find(`.uploaded-img`).css({'object-fit': ''});
                        });
                    } else {
                        $message.find(`div[data-upload-file-id="${file.upload_id}"] img`).attr('data-mfp-src', url);
                        $message.find(`div[data-upload-file-id="${file.upload_id}"] img`).attr('src', url);
                        $message.find(`div[data-upload-file-id="${file.upload_id}"] img`)[0] && ($message.find(`div[data-upload-file-id="${file.upload_id}"] img`)[0].onload = () => {
                            $message.find(`div[data-upload-file-id="${file.upload_id}"] img`).css({'object-fit': ''});
                        });
                ...
                console.error($message);
                if (list.length === 1){
                    $message.find(`.uploaded-img`).attr('data-mfp-src', '');
                    $message.find(`.uploaded-img`).attr('src', '');
                } else {
        ...
        _.each(files, onFileLoop);
        _.each(fwd_images, onFileLoop);
    },

    createImageGrid: function (images) {
    ...
    buildMessageHtml: function (message) {
        ...
        if (this.account.server_features.get('proxy-viewer') && this.account.get('proxy_viewer_url') && this.account.get('proxy_viewer_token') && attrs.type !== 'file_upload'){
            images = this.replaceSourcesToPlaceholder(images);
            proxy_images = true;
        }
        ...
        if (is_image) {
            if (images.length > 1) {
                template_for_images = this.createImageGrid(images);
            }
        }
        ...
        if (attrs.searched_message){
            let myRegexp = new RegExp('(.{0,12})(' + utils.escapeTextToRegexText(attrs.query) + ')(.{0,12})','gmius'),
                    ...
                    if (is_audio && $message.find('.link-file').length) {
                        ...
                        if (file && file.key) {
                            if (this.account.server_features.get('proxy-viewer') && this.account.get('proxy_viewer_url') && this.account.get('proxy_viewer_token')){
                                ...
                                audio_player.$audio_elem.voice_message = this.renderVoiceMessage($message.find('.link-file').find('.file-container')[0], f_url, null, file, file.key);
                                xabber.trigger('plyr_player_updated');
                            } else {
                                this.model.messages.decryptFile(f_url, file.key).then((result) => {
                                    if (result === null)
                                        return;
                                    $message.find('.link-file').find('.mdi-play').removeClass('no-uploaded');
                                    audio_player.$audio_elem.voice_message = this.renderVoiceMessage($message.find('.link-file').find('.file-container')[0], result, null, file);
                            ...
                            $message.find('.link-file').find('.mdi-play').removeClass('no-uploaded');
                            audio_player.$audio_elem.voice_message = this.renderVoiceMessage($message.find('.link-file').find('.file-container')[0], f_url, null, file);
            ...
            if (files.length > 0) {
                $(files).each((idx, item) => {
                    ...
                    if (item.voice){
                        ...
                        this.renderVoiceMessage(template_for_file_content.find('.file-container'), audio, null, item, item.key);
        ...
        if (attrs.forwarded_message && !attrs.searched_message) {
            $(attrs.forwarded_message).each((idx, fwd_msg) => {
                ...
                if (this.account.server_features.get('proxy-viewer') && this.account.get('proxy_viewer_url') && this.account.get('proxy_viewer_token')){
                    images_forward = this.replaceSourcesToPlaceholder(images_forward);
                }

                if (is_image_forward) {
                    if (images_forward.length > 1) {
                        template_for_images = this.createImageGrid(images_forward);
                ...
                if (is_forward_video) {
                    ...
                    if (this.account.server_features.get('proxy-viewer') && this.account.get('proxy_viewer_url') && this.account.get('proxy_viewer_token')){
                        ...
                        updateVideoInMessage(attrs.videos);
                    }
                }
                    ...
                    if (attrs.files.length > 0) {
                        ...
                        $(file_attrs).each((idx, file) => {
                            ...
                            if (is_audio && $f_message.find('.link-file').length) {
                                ...
                                audio_player.$audio_elem.voice_message = this.renderVoiceMessage($f_message.find('.link-file').find('.file-container')[0], f_url, null, file, file.key);
                                xabber.trigger('plyr_player_updated');
                            }
                        });
                    }
                ...
                if (this.account.server_features.get('proxy-viewer') && this.account.get('proxy_viewer_url') && this.account.get('proxy_viewer_token') && attrs.type !== 'file_upload'){
                    this.loadAndReplaceImagesToProxy(null, $f_message, attrs.encrypted || this.model.get('encrypted') || has_encrypted_files, images_forward);
                }
            });
            this.updateScrollBar();
            if (this.model.get('saved') && $message.hasClass('saved-main')) {
        ...
        if ((attrs.encrypted || this.model.get('encrypted') || has_encrypted_files) && !proxy_images) {
    ...
    initPlyrEmbedPlayer: function ($msg, msg) {
        ...
        $msg.find('.plyr-video-container:not(.no-load)').each((idx, item) => {
            ...
            if (existing_player.length){
                ...
                player.proxy_video = $(item).attr('data-proxy');
    ...
    showMessageAuthor: function ($msg) {
        ...
        $avatar.setAvatar(image, this.avatar_size, this.account);
        if ($msg.data('avatar')) {
            if ($msg.data('from-id')) {
                if (this.account.chat_settings.getHashAvatar($msg.data('from-id')) === $msg.data('avatar') && (this.account.chat_settings.getB64Avatar($msg.data('from-id')))) {
                    $avatar.setAvatar(this.account.chat_settings.getB64Avatar($msg.data('from-id')), this.avatar_size, this.account);
                }
                else {
                    let pending_avatar = this._pending_avatars.find(a => a.hash === $msg.data('avatar'));
                    if (pending_avatar) {
                        pending_avatar.dfd.done((data_avatar) => {
                            $avatar.setAvatar(data_avatar, this.avatar_size, this.account);
                        ...
                        if($msg.data('avatar-url')){
                            $avatar.setAvatar($msg.data('avatar-url'), this.avatar_size, this.account);
        ...
        if ($fwd_message.data('avatar')) {
            if ($fwd_message.data('from-id')) {
                if ((this.account.chat_settings.getHashAvatar($fwd_message.data('from-id')) === $fwd_message.data('avatar')) && (this.account.chat_settings.getB64Avatar($fwd_message.data('from-id')))) {
                    $avatar.setAvatar(this.account.chat_settings.getB64Avatar($fwd_message.data('from-id')), this.avatar_size, this.account);
                }
                else {
                    if($fwd_message.data('avatar-url')){
                        $avatar.setAvatar($fwd_message.data('avatar-url'), this.avatar_size, this.account);
            ...
            $(files_).each((idx, item) => {
                ...
                if (is_audio && $message.find('.link-file').length) {
                    ...
                    if (item && item.key) {
                        if (this.account.server_features.get('proxy-viewer') && this.account.get('proxy_viewer_url') && this.account.get('proxy_viewer_token')){
                            $message.find('.link-file').find('.mdi-play').removeClass('no-uploaded');
                            audio_player.$audio_elem.voice_message = this.renderVoiceMessage($message.find('.link-file').find('.file-container')[0], f_url, null, item, item.key);
                            ...
                            this.model.messages.decryptFile(f_url, item.key).then((result) => {
                                if (result === null)
                                    return;
                                $message.find('.link-file').find('.mdi-play').removeClass('no-uploaded');
                                audio_player.$audio_elem.voice_message = this.renderVoiceMessage($message.find('.link-file').find('.file-container')[0], result, null, item);
                                message.set('msg_player_audios', [audio_player]);
                                xabber.trigger('plyr_player_updated');
                            });
                        }
    ...
    onChangedReadState: function (message) {
        ...
        if (is_unread) {
            ...
            let read_timeout = message.get('is_mention_msg') ? 3000 : 1000;
            setTimeout(() => {
                $msg.removeClass('unread-message-background');
            }, read_timeout);
            this.model.recountUnread();
    ...
    handleProxyFileLoad: function (file) {
        ...
        utils.callback_popup_message(xabber.getString("proxy_file_load_download_has_started"), 5000);

        this.account.getProxyUrl(file.sources[0], (response) => {
            if (!response || !response.url) {
                console.error(response);
                return;
            }
        ...
        if (!$elem.hasClass('mdi-link-variant') && !$elem.hasClass('msg-copy-location-content') && !$elem.hasClass('btn-retry-send-message') && !$elem.hasClass('btn-delete-message') && !$elem.hasClass('file-link-download') && !$elem.is('canvas') && !$elem.hasClass('voice-message-volume')) {
            ...
            if ($elem.hasClass('voice-message-play') || $elem.hasClass('no-uploaded')) {
                ...
                if ($elem.closest('.chat-message').hasClass('encrypted')) {
                    ...
                    if (file && file.key) {
                        if (this.account.server_features.get('proxy-viewer') && this.account.get('proxy_viewer_url') && this.account.get('proxy_viewer_token')){
                            ...
                            this.model.messages.decryptFile(f_url, file.key).then((result) => {
                                if (result === null)
                                    return;
                                $audio_elem[0].voice_message = this.renderVoiceMessage($audio_elem.find('.file-container')[0], result, null, file);
                            });
                        }
                    }
                } else {
                    ...
                    $audio_elem[0].voice_message = this.renderVoiceMessage($audio_elem.find('.file-container')[0], f_url, null, file);
                }
                return;
            }
...
xabber.AccountChats = xabber.ChatsBase.extend({
    ...
    onCompletedMAMRequest: function () {
        this.mam_requests--;
        this.runMAMRequests();
    },
    ...
    receiveMessage: function (message) {
        ...
        this.makeMessageObject(message, {realtime_msg: true});
    },
    ...
    makeMessageObject: async function (message, options) {
        ...
        _.extend(msg_object, options);
        if ($mam.length && msg_object.realtime_msg && (!type || type === 'chat' || (type === 'normal')))
            return;

        if (!msg_object.type){
            if (type === 'headline') {
                msg_object.type = 'headline';
            } else {
        ...
        if (!chat){
            console.error('!NO CHAT OBJECT ON THIS MSG');
        } else {
            return chat.receiveMessage(msg_object);
        }
    },
...
xabber.AddGroupChatView = xabber.SearchView.extend({
    ...
    showPlaceholder: function () {
        let textarea_is_empty = (this.$('.rich-textarea ').text() === "");
...
xabber.ChatsView = xabber.SearchPanelView.extend({
    ...
    updateChatPositionDebounced: function (item, unread) {
        let view,
            active_toolbar = xabber.toolbar_view.$('.active'),
            is_unread_forced = !item && xabber.toolbar_view.$('.active.unread').length;
        if (item){
            view = this.child(item.id);
            if (item && active_toolbar.hasClass('unread') && (item.get('notifications') || !(item.get('unread') || item.get('const_unread')))) {
                return;
            }
            if (view && active_toolbar.hasClass('account-item') && view.account.get('jid') !== active_toolbar.attr('data-jid')){
                return;
            }
    ...
    fillIfChatsEmpty: function () {
        if (!this.$('.chat-item').length)
    ...
    showGroupChats: function () {
        ...
        let chats = this.model,
            ...
            group_chats_pinned = [];
        if (is_unread) {
            group_chats = chats.filter(chat => chat.contact && chat.contact.get('group_chat') && chat.get('timestamp') && (!chat.get('archived') && !chat.get('notifications')) && chat.last_message && !chat.last_message.get('invite') && (chat.get('unread') || chat.get('const_unread')) && (chat.get('pinned') === '0' || !chat.get('pinned')));
            group_chats_pinned = chats.filter(chat => chat.contact && chat.contact.get('group_chat') && chat.get('timestamp') && (!chat.get('archived') && !chat.get('notifications')) && chat.last_message && !chat.last_message.get('invite') && (chat.get('unread') || chat.get('const_unread')) && chat.get('pinned') !== '0' && chat.get('pinned'));
        }
        if (!group_chats.length && !group_chats_pinned.length) {
            group_chats = chats.filter(chat => !chat.get('saved') && chat.contact.get('group_chat') && chat.get('timestamp') && (!chat.get('archived') && !chat.get('notifications')) && (chat.get('pinned') === '0' || !chat.get('pinned')));
            group_chats_pinned = chats.filter(chat => !chat.get('saved') && chat.contact.get('group_chat') && chat.get('timestamp') && (!chat.get('archived') && !chat.get('notifications')) && chat.get('pinned') !== '0' && chat.get('pinned'));
    ...
    showChats: function () {
        ...
        if (is_unread) {
            private_chats = chats.filter(chat => chat.contact && !chat.contact.get('group_chat') && chat.get('timestamp') && (!chat.get('archived') && !chat.get('notifications')) && chat.last_message && !chat.last_message.get('invite') && (chat.get('unread') || chat.get('const_unread')) && (chat.get('pinned') === '0' || !chat.get('pinned')));
            private_chats_pinned = chats.filter(chat => chat.contact && !chat.contact.get('group_chat') && chat.get('timestamp') && (!chat.get('archived') && !chat.get('notifications')) && chat.last_message && !chat.last_message.get('invite') && (chat.get('unread') || chat.get('const_unread')) && chat.get('pinned') !== '0' && chat.get('pinned'));
        }
        if (!private_chats.length && !private_chats_pinned.length) {
            private_chats = chats.filter(chat => !chat.get('saved') && !chat.contact.get('group_chat') && chat.get('timestamp') && (!chat.get('archived') && !chat.get('notifications')) && (chat.get('pinned') === '0' || !chat.get('pinned')));
            private_chats_pinned = chats.filter(chat => !chat.get('saved') && !chat.contact.get('group_chat') && chat.get('timestamp') && (!chat.get('archived') && !chat.get('notifications')) && chat.get('pinned') !== '0' && chat.get('pinned'));
            xabber.toolbar_view.$('.toolbar-item:not(.toolbar-logo).unread').removeClass('unread');
            this.onUpdatedScreen();
        }
        private_chats.forEach((chat) => {
    ...
    showChatsByAccount: function (account) {
        ...
        this.$('.chat-item').detach();
        let chats = this.model,
            account_chats = chats.filter(chat => ((chat.account.get('jid') === account.get('jid')) && (!chat.get('saved') && chat.get('timestamp') && (!chat.get('archived') && !chat.get('notifications')))) && (chat.get('pinned') === '0' || !chat.get('pinned'))),
            account_chats_pinned = chats.filter(chat => ((chat.account.get('jid') === account.get('jid')) && (!chat.get('saved') && chat.get('timestamp') && (!chat.get('archived') && !chat.get('notifications')))) && chat.get('pinned') !== '0' && chat.get('pinned'));
    ...
    search: function (query) {
        ...
        query && this.account.roster.forEach((contact) => {
            let jid = contact.get('jid'),
                ...
                is_match = (name.indexOf(query) < 0 && jid.indexOf(query) < 0);
            if (!is_match) {
        ...
        this.$('.chat-item').each((idx, item) => {
            ...
            is_match = (!(!name || name && (name.indexOf(query) < 0 && jid.indexOf(query) < 0)));
...
xabber.InvitationPanelView = xabber.SearchView.extend({
  ...
  xabber.SavedChatHeadView = xabber.BasicView.extend({
      ...
      renderSearchPanel: function () {
          this.showSavedDetailsRight();
      },

      showSavedDetailsRight: function () {
          let chat = this.model,
          ...
          let attrs = {
              right_contact: 'contact_details_saved',
              contact: 'saved',
              saved_chat_search: true,
          };
  ...
  xabber.ChatHeadView = xabber.BasicView.extend({
          ...
          this.account.sendFast(stanza, () => {
              ...
              this.account.omemo.xabber_trust.addVerificationSessionData(sid, {
                  ...
                  active_verification_device: {
                      peer_jid: this.contact.get('jid'),
                  },
                  verification_step: '1a',
                  session_check_jid: this.contact.get('jid'),
                  msg_ttl: 86400,
    ...
    updateMenu: function () {
        let is_group_chat = this.contact.get('group_chat'),
            is_notifications = this.model.get('notifications');
    ...
    archiveChat: function (ev, no_iq) {
        let archived = !this.model.get('archived');
        if (!no_iq) {
            let is_archived_status = archived ? 'archived' : 'active',
                conversation_options = {
                    jid: this.contact.get('jid'),
  ...
  xabber.SendMediaView = xabber.BasicView.extend({
      ...
      createFileFromURL: async function (url, filename) {
          ...
          return new File([data], filename || url.split('#').shift().split('?').shift().split('/').pop() || 'file', metadata)
      },
...
xabber.ChatLocationView = xabber.BasicView.extend({
    ...
    initMap: function () {
        import('ol-local').then(ol => {
            ...
            let getCurrentPositionControl = function () {
            ...
            if (xabber.popup_coordinates) {
                ...
                let getPlacemarkPositionControl = function () {
...
xabber.ChatBottomView = xabber.BasicView.extend({
    ...
    _initialize: function (options) {
        ...
        let bindings = {
            ...
            arrow_left: {
                ...
                handler: () => {
                    return this.$('.mentions-list').css('display') === 'none';
                }
            },
            arrow_right: {
    ...
    updateAvatar: function () {
        ...
        if (this.contact && this.contact.get('group_chat')) {
            ...
            image = this.account.cached_image;
        this.$('.my-avatar.circle-avatar').setAvatar(image, this.avatar_size, this.account);
    },
    ...
    onChangedText: function () {
        ...
        let text = quill_textarea.getTextFromRichTextarea().replace(/\n$/, "");
        ...
        this.contact.searchByParticipants(mention_text, (participants) => {
            if (participants.length || xabber.getString("chat_bottom__mentions_list__item_everyone").toLowerCase().indexOf(mention_text) > -1 || mention_text === "*" || 'all'.indexOf(mention_text) > -1 || 'все'.indexOf(mention_text) > -1) {
                ...
                participants.forEach((participant) => {
                    ...
                    mention_item.find('.circle-avatar').setAvatar(participant.get('b64_avatar') || utils.images.getDefaultAvatar(participant.get('nickname') || participant.get('jid') || participant.get('id')), this.mention_avatar_size, this.account);
                ...
                mention_all.find('.circle-avatar').setAvatar(this.contact.cached_image, this.mention_avatar_size, this.account);
    ...
    removeAllFileSnippets: function () {
    ...
    deleteFilesFromMessages: function (messages) {
        messages.forEach((item) => {
            ...
            item.get('files') && _.isArray(item.get('files')) && item.get('files').forEach((item) => {
                item && item.sources && item.sources.length && this.account.deleteFileByUrl(item.sources[0],() => {
                    item.sources = null;
                }, () => {
                    item.sources = null;
                });
...
xabber.ChatBodyContainer = xabber.Container.extend({
    ...
    updateBodyNotifications: function (chat) {
        if (chat.get('notifications')){
            ...
            this.$el.removeClass('chat-body-container-notifications');
        }
    }
});
...
xabber.ChatSettings = Backbone.ModelWithStorage.extend({
    ...
    getAvatarInfoById: function (id) {
        let avatar_list = _.clone(this.get('cached_avatars'));
        return avatar_list.find(member => member.id === id);
    },
