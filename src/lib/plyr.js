typeof navigator === "object" && (function (global, factory) {
  ...
  function triggerEvent(element, type = '', bubbles = false, detail = {}) {
    ...
    const event = new CustomEvent(type, {
      bubbles,
      detail: Object.assign({}, detail, { plyr: this }),
    }); // Dispatch the event
  ...
  const standardRatios = [[1, 1], [4, 3], [3, 4], [5, 4], [4, 5], [3, 2], [2, 3], [16, 10], [10, 16], [16, 9], [9, 16], [21, 9], [9, 21], [32, 9], [9, 32]].reduce(
      (out, [x, y]) => {return Object.assign({}, out, { [x / y]: [x, y] })}, {}); // Validate an aspect ratio
  ...
  const controls = {
    ...
    createLabel(key, attr = {}) {
      ...
      const attributes = Object.assign({}, attr, { class: [attr.class, this.config.classNames.hidden].filter(Boolean).join(' ')});
  ...
  const vimeo = {
    setup() {
      ...
      //modified this to work with require js
      if (!is.object(window.Vimeo)) {
        ...
        var vimeoTimer = window.setInterval(function () {
          if (is.object(window.Vimeo)) {
            ...
            require([player.config.urls.vimeo.sdk], function (req_player) {
              window.Vimeo = { Player: req_player };
              vimeo.ready.call(player);
            });
            window.clearInterval(vimeoTimer);
          }
    ...
    ready() {
      ...
      const referrerPolicy = config.referrerPolicy;
      const frameParams = config;
      ...
      Object.assign(temp_params, hashParam);
      Object.assign(temp_params, frameParams);
      const params = buildUrlParams(temp_params);
