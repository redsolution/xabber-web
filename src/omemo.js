...
xabber.Peer = Backbone.Model.extend({
    ...
    getDevice: function (id) {
        if (!this.devices[id]) {
            this.devices[id] = new xabber.Device({jid: this.get('jid'), id: id }, { account: this.account, store: this.store, peer: this});
        }
...
xabber.Fingerprints = createVueBackboneView(xabber, {
    ...
    events: {
        ...
        'contextmenu .btn-fingerprint-details': "onDeviceContextMenu",
    ...
    onDeviceContextMenu: function (ev) {
        let $tab = $(ev.target).closest('.btn-fingerprint-details'),
            device_id = $tab.attr('data-device-id');
        ...
        $modal.find('.btn-verify').addClass('hidden');
        $modal.find('.btn-terminate-session').addClass('hidden');

        $modal.find('.btn-trust').one(`click.${unique_modal_id}`, () => {
            if (device){
                this.device_id = device_id;
                this.fingerprint = $tab.attr('data-fingerprint').replace(/ /g, "");
                this.trust = $tab.attr('data-trust');
                this.trustDevice({target: this.$('.contact-device-content')});
            }
            $overlay.click();
        });
            ...
            if (this.omemo.xabber_trust){
                this.omemo.xabber_trust.getTrustedKey(device).then((trustedKeyBuffer) => {
                    let trustedKeyBase64 = utils.ArrayBuffertoBase64(trustedKeyBuffer),
                        ...
                        to = this.jid, changed, changed_devices = {};
    ...
    addRow: function (id, label, trust, fingerprint, options) {
        ...
        return templates.fingerprint_item({id,label,trust, trust_type,fingerprint, delete_button, edit_setting, old_fingerprint, error});
    },
...
xabber.FingerprintsOwnDevices = createVueBackboneView(xabber, {
    ...
    open: function (device_id, is_own, callback) {
        this.omemo = this.account.omemo;
        this.data.set('visible', true);
        this.updateColorScheme();
        this.show();
        this.renderOwnDevices(device_id, is_own, callback);
    },
    ...
    renderTrustOnFingerprint: function (is_trusted, $container, context, callback) {
        ...
        if (is_trusted !== 'trust' && is_trusted !== 'ignore'){
            ...
            if (is_trusted === 'error'){
                $warning_container.addClass('warning-error');
                ...
                $warning_container.find('.device-encryption-warning-name').html(xabber.getQuantityString("settings_account__encryption_warning_header", [untrusted_count]));
    ...
    renderOwnDevices: function (device_id, is_own, callback) {
        this.device_id = device_id;
        ...
        if (is_own)
            ...
            this.updateOwnFingerprint(device_id);
            this.updateFingerprints(device_id, callback);
        }
    },
    ...
    updateFingerprints: async function (device_id, callback) {
    ...
    addRow: function (id, label, trust, fingerprint, options) {
        ...
        return templates.fingerprint_devices_item({id,label,trust, svg_icon, fingerprint, edit_setting, old_fingerprint, error});
    },
...
xabber.Prekeys = Backbone.Model.extend({
    ...
    getAll: function () {
        return _.clone(this.model.get(this.name));
    },
...
xabber.Device = Backbone.Model.extend({
    initialize: function (attrs, options) {
        ...
        this.jid = attrs.jid;
        this.store = options.store;
        options.peer && (this.peer = options.peer);
        this.preKeys = null;
    ...
    getPreKey: function () {
        return this.account.omemo.used_prekeys.get(String(this.id));
    },
...
xabber.Omemo = Backbone.ModelWithStorage.extend({
    ...
    encrypt: function (contact, message, is_own) {
        ...
        is_own && this.cached_messages.putMessageOwn(this.account.get('jid'), origin_id, {envelope: plaintext});
        ...
        return peer.encrypt(plaintext).then((encryptedMessage) => {
            ...
            for (let key of encryptedMessage.keys) {
                ...
                if (contact){
                    ...
                    if (peer.devices[key.deviceId])
                        contact && encryptedElement.c('key', attrs).t(btoa(key.ciphertext.body)).up();
                    else
            ...
            contact && encryptedElement.up();
            encryptedElement.cnode(myKeys.tree());
...
xabber.DecryptedMessages = Backbone.ModelWithStorage.extend({
    ...
    getMessages: function (contact) {
        let messages = _.clone(this.get('messages'));
        return messages[contact.get('jid')] || {};
    },
